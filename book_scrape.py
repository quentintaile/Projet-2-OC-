...
# Paramètres : url_produit : Lien vers la page produit du livre
def extraire_donnees_livre(url_produit):
    ...
    prix_ttc = soupe_produit.find('th', string='Price (incl. tax)').find_next_sibling('td').text.replace('£', '').strip() if soupe_produit.find('th', string='Price (incl. tax)') else 'Non disponible'
    prix_ht = soupe_produit.find('th', string='Price (excl. tax)').find_next_sibling('td').text.replace('£', '').strip() if soupe_produit.find('th', string='Price (excl. tax)') else 'Non disponible'
    ...
    # Retourner les informations avec les clés en français
    return {
        'URL page produit': url_produit,
        'Code UPC': code_produit,
        'Titre': soupe_produit.h1.text,
        'Prix TTC': float(prix_ttc) if prix_ttc != 'Non disponible' else prix_ttc,
        'Prix HT ': float(prix_ht) if prix_ht != 'Non disponible' else prix_ht,
        'Disponibilite': disponibilite,
        'Description du produit': description,
        'Categorie': categorie,
        'Evaluation': note,
        'URL de l\'image': url_miniature
    }
...
# Fonction pour sauvegarder l'image miniature dans un dossier spécifique de catégorie
def sauvegarder_image_miniature(url, titre, categorie):
    # Créer le dossier de la catégorie dans 'images' si nécessaire
    chemin_dossier_categorie = os.path.join('images', categorie)
    os.makedirs(chemin_dossier_categorie, exist_ok=True)

    reponse = requests.get(url)
    titre_sanitaire = "".join([c if c.isalnum() else "_" for c in titre])
    chemin_image = os.path.join(chemin_dossier_categorie, f"{titre_sanitaire}.jpg")
    with open(chemin_image, 'wb') as fichier:
        fichier.write(reponse.content)
    return chemin_image

# Fonction pour récupérer les livres de toutes les pages d'une catégorie
def extraire_livres_par_categorie(url_categorie, nom_categorie):
...
for nom_categorie, lien_categorie in categories:
    ...
    livres = extraire_livres_par_categorie(lien_categorie, nom_categorie)
    
    # Sauvegarder les données dans un fichier CSV pour chaque catégorie avec point-virgule comme séparateur
    if livres:
